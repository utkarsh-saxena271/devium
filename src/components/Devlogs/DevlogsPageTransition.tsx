...
function isDevlogDateLink(href: string) {
  const parts = href.split("/").filter(Boolean)
  return parts[0] === "devlogs" && parts.length === 4
}

export default function DevlogsPageTransition({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const handleClick = (e: MouseEvent) => {
      ...
      if (href && isDevlogDateLink(href)) setShowSkeleton(true)
    }
